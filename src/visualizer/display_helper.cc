...
void DisplayHelper::DisplayInfoForState(double percent,
...
void DisplayHelper::DrawLoadBar(double percent) const {
  vec2 progress_bottom_right =
      vec2(loading_bar_bottom_right_.x * percent, loading_bar_bottom_right_.y);
  ci::gl::color(kProgressBarColor);
  ci::gl::drawSolidRect(ci::Rectf(loading_bar_top_left_, progress_bottom_right));

  ci::gl::color(kBorderColor);
  ci::gl::drawStrokedRect(ci::Rectf(loading_bar_top_left_, loading_bar_bottom_right_));

}

void DisplayHelper::DrawMessage(const string& message) const {
  ci::gl::color(kBorderColor);
  ci::gl::drawStrokedRect(ci::Rectf(message_box_top_left_, message_box_bottom_right_));
...
void DisplayHelper::DrawKey(const string& key) const {
  ci::gl::color(kBorderColor);
  ci::gl::drawStrokedRect(ci::Rectf(key_box_top_left_, key_box_bottom_right_));
...
void DisplayHelper::DrawStepBox(const string& step) const {
  ci::gl::color(kBorderColor);
...
void DisplayHelper::DrawMainButtons() const {
  ci::gl::color(kBorderColor);
  ci::gl::drawStrokedRect(ci::Rectf(main_buttons_top_left_, main_buttons_bottom_right_));
...
void DisplayHelper::DrawInstructions() const {
  ci::gl::color(kBorderColor);
  ci::gl::drawStrokedRect(ci::Rectf(instructions_top_left_, instructions_bottom_right_));
...
vec2 DisplayHelper::TextLocation(const vec2& top_left,
...
void DisplayHelper::DisplayText(const vec2& top_left,
  ...
  ci::gl::drawStringCentered(text,
                             TextLocation(top_left, bottom_right),
                             kBorderColor,
                             font);
}
