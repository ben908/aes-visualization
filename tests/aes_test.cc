...
TEST_CASE("debugging") {
  ...
  unsigned char desired_result[] = { 0x69, 0xc4, 0xe0, 0xd8, 0x6a, 0x7b, 0x04, 0x30, 0xd8, 0xcd, 0xb7, 0x80, 0x70, 0xb4, 0xc5, 0x5a };
  unsigned char result_location[16];
  
  aes.Encrypt(message, result_location, key);
  for (size_t i = 0; i < 4; i++) {
    for (size_t j = 0; j < 4; ++j) {
      printf("%02x ", result_location[i + j]);
      std::cout << " ";
    }
    std::cout << std::endl;
  }
  REQUIRE(0 == memcmp(desired_result, result_location, block_memory_size));
}
...
TEST_CASE("128-bit known key-text ciphers") {
  ...
  unsigned char message[] = {0xf3, 0x44, 0x81, 0xec, 0x3c, 0xc6, 0x27, 0xba, 0xcd, 0x5d, 0xc3, 0xfb, 0x08, 0xf2, 0x73, 0xe6};
...
TEST_CASE("Test Finite Multiplication") {
  REQUIRE(AES::FiniteMultiply(0xae, 0x47) == 0x01);
  REQUIRE(AES::FiniteMultiply(0x53, 0xca) == 0x01);
  REQUIRE(AES::FiniteMultiply(0x53, 0xca) == 0x01);  
  REQUIRE(AES::FiniteMultiply(0x53, 0xca) == 0x01);
  REQUIRE(AES::FiniteMultiply(0x32, 0x32) == 0xC4);
}
