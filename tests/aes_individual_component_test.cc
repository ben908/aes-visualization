#include <core/aes.h>

#include <catch2/catch.hpp>
#include <iostream>

namespace aes {

TEST_CASE("128 bit Key Expansion") {
  AES aes = AES(128);
  unsigned char key[] =
      {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88,
       0x09, 0xcf, 0x4f, 0x3c};
  aes.SetKey(key);
  
  unsigned char known_key_expansion[] = {
      
  };

  unsigned char* calculated_key_expansion = aes.GetKeyExpansion();

  size_t block_memory_size = 43 * 4 * sizeof(unsigned char);
  REQUIRE(0 == memcmp(known_key_expansion,
                      calculated_key_expansion,
                      block_memory_size));
}

TEST_CASE("192 bit Key Expansion") {
  AES aes = AES(192);
  unsigned char key[] ={
      0x8e, 0x73, 0xb0, 0xf7, 0xda, 0x0e, 0x64, 0x52, 0xc8, 0x10, 0xf3, 0x2b,
      0x80, 0x90, 0x79, 0xe5, 0x62, 0xf8, 0xea, 0xd2, 0x52, 0x2c, 0x6b, 0x7b
  };
  aes.SetKey(key);
  
  unsigned char known_key_expansion[] = {
      
  };
  
  unsigned char* calculated_key_expansion = aes.GetKeyExpansion();
  size_t block_memory_size = 51 * 4 * sizeof(unsigned char);

  REQUIRE(0 == memcmp(known_key_expansion,
                      calculated_key_expansion,
                      block_memory_size));
}

TEST_CASE("256 bit Key Expansion") {
  AES aes = AES(256);
  unsigned char key[] ={
      0x60, 0x3d, 0xeb, 0x10, 0x15, 0xca, 0x71, 0xbe, 0x2b, 0x73, 0xae, 0xf0,
      0x85, 0x7d, 0x77, 0x81, 0x1f, 0x35, 0x2c, 0x07, 0x3b, 0x61, 0x08, 0xd7,
      0x2d, 0x98, 0x10, 0xa3, 0x09, 0x14, 0xdf, 0xf4
  };
  aes.SetKey(key);

  unsigned char known_key_expansion[] = {
      
  };

  unsigned char* calculated_key_expansion = aes.GetKeyExpansion();

  size_t block_memory_size = 59 * 4 * sizeof(unsigned char);
  REQUIRE(0 == memcmp(known_key_expansion,
                      calculated_key_expansion,
                      block_memory_size));
}



TEST_CASE("Test Finite Multiplication") {
  REQUIRE(AES::FiniteMultiply(0xae, 0x47) == 0x5F);
  REQUIRE(AES::FiniteMultiply(0x53, 0xca) == 0x01);
  REQUIRE(AES::FiniteMultiply(0xff, 0xff) == 0x13);
  REQUIRE(AES::FiniteMultiply(0xa3, 0x3a) == 0x4b);
  REQUIRE(AES::FiniteMultiply(0x32, 0x32) == 0x73);
}

} //namespace aes